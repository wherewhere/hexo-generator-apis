 ...
 * @property {boolean} search_all.enable
 * @property {string} search_all.path
 * @property {boolean} search_all.cover
 * @property {boolean} search_all.excerpt
 * @property {boolean} search_all.content
 */
...
module.exports = function (hexo, site) {
  ...
  /**
   * @typedef {{perPage: number, data: {name: string, path: string, postlist: postMap[]}}} cate
   */
   ...
   * @property {string} content
   * @property {string} raw
   * @property {{name: string, path: string}[]} categories
   * @property {{name: string, path: string}[]} tags
   */
  ...
  if (restful.posts_size > 0) {
    ...
    const page_posts = [],
      /**
       * @type {{index: number, path: string, count: number}[]}
       */
      pages_data = [],
  ...
  if (!!restful.swipers_list?.length) {
    ...
    const res = [];
    swipier.forEach(i => {
      res.push(...postlist.filter(v => v.cover && v.slug == i))
    })
