...
/** @typedef {import("./hexo").Hexo} Hexo */
/** @typedef {import("./hexo").SiteLocals} SiteLocals */
/** @typedef {import("./hexo").SiteLocals.Posts} SitePosts */
/** @typedef {import("./hexo").BaseGeneratorReturn} BaseGeneratorReturn */
/** @typedef {import("./hexo").PostSchema} PostSchema */
/** @typedef {import("./hexo").CategorySchema} CategorySchema */
/** @typedef {import("./hexo").TagSchema} TagSchema */

/** @typedef {import("./types").Config} Config */
/** @typedef {import("./types").PostsProps} PostsProps */
/** @typedef {import("./types").CategoryGenerator} CategoryGenerator */
/** @typedef {import("./types").IndexGenerator} IndexGenerator */
/** @typedef {import("./types").ArchiveGenerator} ArchiveGenerator */
/** @typedef {import("./types").SearchAll} SearchAll */
/** @typedef {import("./types").OpenApiInfoContact} OpenApiInfoContact */
/** @typedef {import("./types").OpenApiInfoLicense} OpenApiInfoLicense */
/** @typedef {import("./types").OpenApiInfo} OpenApiInfo */
/** @typedef {import("./types").OpenApiExternalDocs} OpenApiExternalDocs */
/** @typedef {import("./types").OpenApiServer} OpenApiServer */
/** @typedef {import("./types").OpenApi} OpenApi */
/** @typedef {import("./types").RestfulXApi} RestfulXApi */
/** @typedef {import("./types").Cate} Cate */
...
module.exports = function (hexo, site) {
    ...
    /**
     * @param {string} path
     * @param {SitePosts} posts
     * @param {number} page_size
     */
    function generate(path, posts, page_size, globalInfo = {}, baseInfo = {}) {
        const postlist = posts.map(x => postMap(x, restful.posts_props));
        if (page_size > 0) {
            /** @type {BaseGeneratorReturn[]} */
            const page_posts = [],
    ...
    /**
     * @param {CategorySchema[] | TagSchema[]} cates
     * @param {string} kind
     * @returns {Cate[]}
     */
    ...
    /**
     * @param {PostSchema} post
     * @param {PostsProps} props
     */
    function postMap(post, props) {
        /** @type {Partial<PostSchema> & Pick<PostSchema, 'title' | 'slug' | 'date' | 'updated'>} */
        const result = {
            title: post.title,
    ...
    if (restful.posts?.enable) {
        ...
        if (restful.posts.archive_generator?.enable) {
            ...
            /** @type {Record<number, (PostSchema[] & { day: Record<number, PostSchema[]> })[]>} */
            const posts = {};
    ...
    if (restful.swipers_list?.length) {
        ...
        /** @type {PostSchema[]} */
        const resources = [];
        swipier.forEach(i => resources.push(...posts.filter(v => v.slug == i && (v.cover ?? v.banner)).map(x => postMap(x, { ...restful.posts_props, cover: true }))));
