 ...
 * @param {number} index
 */
function getPostsApi(index) {
  return "api/posts/" + index + ".json";
}
...
function getPostApi(post) {
  return "api/posts/" + getJsonPath(post.path);
}
...
module.exports = function (hexo, site) {
  ...
  function cateReduce(cates, kind) {
    return cates.reduce((result, cate) => {
      if (!cate.length) {
        return result;
      }
      return result.concat(pagination(cate.path, posts, {
        perPage: 0,
        data: {
          name: cate.name,
          postlist: cate.posts.map(postMap),
          api: getCateApi(kind, cate)
        }
      }));
    }, []);
  }
  ...
  if (restful.categories) {
    ...
    if (cates.length) {
      ...
      fileList = fileList.concat(cates.map(cateMap));
    }
    apiList.push({
      name: "categories",
      api: categoriesApi
  ...
  if (restful.tags) {
    ...
    if (tags.length) {
      ...
      fileList = fileList.concat(tags.map(cateMap));
    }
    apiList.push({
      name: "tags",
      api: tagsApi
  ...
  if (restful.post) {
    ...
    if (restful.posts_size > 0) {
      ...
      fileList = fileList.concat(page_posts);
    }
    else {
      fileList.push(createFile(
        postsApi,
    ...
    fileList = fileList.concat(posts.map(post => {
  ...
  if (restful.swipers_list?.length) {
    ...
    fileList = fileList.push(createFile(
      swiperApi,
      JSON.stringify(res)
    ));
  }
  ...
  if (restful.search_all?.enable) {
    ...
    fileList = fileList.concat(createFile(
      path,
      resource));
    apiList.push({
      name: "search",
