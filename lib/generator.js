...
module.exports = function (hexo, site) {
  ...
  let restful = cfg.hasOwnProperty('restful_xapi') ? cfg.restful_xapi :
    {
      site: true,
      posts_size: 10,
      posts_props: {
      ...
      return {
        ...
        categories: posts_props('categories', function () {
          return post.categories.map(function (cat) {
            let name = (
              cfg.restful_xapi.use_category_slug && cat.slug
            ) ? cat.slug : cat.name;
            return {
              name: name,
        ...
        tags: posts_props('tags', function () {
          return post.tags.map(function (tag) {
            let name = (
              cfg.restful_xapi.use_tag_slug && tag.slug
            ) ? tag.slug : tag.name;
            return {
              name: name,
    ...
    cateReduce = function (cates, kind) {
      return cates.reduce(function (result, item) {
        ...
        switch (kind) {
          case 'categories':
            use_slug = cfg.restful_xapi.use_category_slug;
            break;
          case 'tags':
            use_slug = cfg.restful_xapi.use_tag_slug;
            break;
        }
  ...
  if (restful.search_all.enable) {
    ...
    let cover = searchConfig.cover;
    let excerpt = searchConfig.excerpt;
    let index = 0
    let res = new Array()
    let posts = site.posts.sort('-date');
    if (posts) {
